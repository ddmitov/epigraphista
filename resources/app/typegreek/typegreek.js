...
function convertCharToggle(control, toggle, event) {
    if (toggle === true) {
        ...
        if (event.ctrlKey || event.metaKey) {
            ...
            if (key == 64) {
                insertAtCursor(control, '̣');
                typeLetter = false;
            }
            if (key == 65) {
                insertAtCursor(control, 'Α');
                typeLetter = false;
            }
            if (key == 66) {
                insertAtCursor(control, 'Β');
                typeLetter = false;
            }
            if (key == 67) {
                insertAtCursor(control, 'Ξ');
                typeLetter = false;
            }
            if (key == 68) {
                insertAtCursor(control, 'Δ');
                typeLetter = false;
            }
            if (key == 69) {
                insertAtCursor(control, 'Ε');
                typeLetter = false;
            }
            if (key == 70) {
                insertAtCursor(control, 'Φ');
                typeLetter = false;
            }
            if (key == 71) {
                insertAtCursor(control, 'Γ');
                typeLetter = false;
            }
            if (key == 72) {
                insertAtCursor(control, 'Η');
                typeLetter = false;
            }
            if (key == 73) {
                insertAtCursor(control, 'Ι');
                typeLetter = false;
            }
            if (key == 74) {
                insertAtCursor(control, 'Σ');
                typeLetter = false;
            }
            if (key == 75) {
                insertAtCursor(control, 'Κ');
                typeLetter = false;
            }
            if (key == 76) {
                insertAtCursor(control, 'Λ');
                typeLetter = false;
            }
            if (key == 77) {
                insertAtCursor(control, 'Μ');
                typeLetter = false;
            }
            if (key == 78) {
                insertAtCursor(control, 'Ν');
                typeLetter = false;
            }
            if (key == 79) {
                insertAtCursor(control, 'Ο');
                typeLetter = false;
            }
            if (key == 80) {
                insertAtCursor(control, 'Π');
                typeLetter = false;
            }
            if (key == 81) {
                insertAtCursor(control, 'Θ');
                typeLetter = false;
            }
            if (key == 82) {
                insertAtCursor(control, 'Ρ');
                typeLetter = false;
            }
            if (key == 83) {
                insertAtCursor(control, 'Σ');
            ...
            if (key == 84) {
                insertAtCursor(control, 'Τ');
                typeLetter = false;
            }
            if (key == 85) {
                insertAtCursor(control, 'Υ');
                typeLetter = false;
            }
            if (key == 86) {
                insertAtCursor(control, 'Ϝ');
                typeLetter = false;
            }
            if (key == 87) {
                insertAtCursor(control, 'Ω');
                typeLetter = false;
            }
            if (key == 88) {
                insertAtCursor(control, 'Χ');
                typeLetter = false;
            }
            if (key == 89) {
                insertAtCursor(control, 'Ψ');
                typeLetter = false;
            }
            if (key == 90) {
                insertAtCursor(control, 'Ζ');
                typeLetter = false;
            }
            if (key == 92) {
                insertAtCursor(control, '`');
            ...
            if (COMBINABLE.indexOf(workingString) > -1) {
                ...
                while ((keepGoing === true) && (startString !== "")) {
                    if ((startString.charAt(0) == "´") || (startString.charAt(0) == "`")) {
                        ...
                        if ((VOWELS.indexOf(workingString.charAt(0)) > -1) &&
                        ...
                        if ((startString.charAt(0) == "῀")) {
                            ...
                            if ((LONG_VOWELS.indexOf(workingString.charAt(0)) > -1) && (workingString.indexOf('´') == -1) &&
                            (workingString.indexOf('`') == -1) &&
                            (workingString.indexOf('῀') == -1)) {
                                ...
                                if ((CAPITALS.indexOf(workingString.charAt(0)) > -1)) {
                                    ...
                                    if ((DIERESIS.indexOf(workingString.charAt(0)) > -1)) {
                                        ...
                                        if ((IOTA.indexOf(workingString.charAt(0)) > -1)) {
                                            ...
                                            (startString.charAt(1) == '᾿') ||
                                            (startString.charAt(1) == '῾'))) ||
                                            ((((workingString.indexOf('᾿') == -1) &&
                                            (workingString.indexOf('῾') == -1) &&
                                            (startString.charAt(1) == 'ͺ') &&
                                            (workingString.indexOf('ͺ') == -1) &&
                                            ((startString.charAt(2) == '᾿') ||
                                            (startString.charAt(2) == '῾')))))) {
                            ...
                            if ((startString.charAt(0) == "᾿") || (startString.charAt(0) == "῾")) {
                                // the next character is ) or (
                                if ((ROUGH_BREATHING.indexOf(workingString.charAt(0)) > -1) && (workingString.indexOf('᾿') == -1) &&
                                (workingString.indexOf('῾') == -1) && 
                                (workingString.indexOf('¨') == -1)) {
                                    ...
                                    if (startString.charAt(0) == "ͺ") {
                                        ...
                                        if ((IOTA.indexOf(workingString.charAt(0)) > -1) && (workingString.indexOf('ͺ') == -1)) {
                                            ...
                                            if (CAPITALS.indexOf(workingString.charAt(0)) > -1) {
                                                // the current string starts with a capital letter
                                                if (((workingString.indexOf('´') == -1) && (workingString.indexOf('`') == -1) &&
                                                (workingString.indexOf('῀') == -1)) || ((workingString.indexOf('᾿') > -1) ||
                                                (workingString.indexOf('῾') > -1) ||
                                                startString.charAt(1) == '᾿' ||
                                                startString.charAt(1) == '῾')) {
                ...
                if (workingString.length > 1) {
                    combineSingleCharacter(control);
                }
...
function combineSingleCharacter(control) {
    ...
    if (workingString.indexOf('´') != -1) {
        ...
        if (workingString.indexOf('᾿') != -1) {
            ...
            if (workingString.indexOf('ͺ') != -1) {
                // if the string contains |
                if (workingString.charAt(0) == "α") {
                    workingString = "ᾄ";
                }
                if (workingString.charAt(0) == "Α") {
                    workingString = "ᾌ";
                }
                if (workingString.charAt(0) == "η") {
                    workingString = "ᾔ";
                }
                if (workingString.charAt(0) == "Η") {
                    workingString = "ᾜ";
                }
                if (workingString.charAt(0) == "ω") {
                    workingString = "ᾤ";
                }
                if (workingString.charAt(0) == "Ω") {
                    workingString = "ᾬ";
                ...
                if (workingString.charAt(0) == "ε") {
                    workingString = "ἔ";
                }
                if (workingString.charAt(0) == "Ε") {
                    workingString = "Ἔ";
                }
                if (workingString.charAt(0) == "ι") {
                    workingString = "ἴ";
                }
                if (workingString.charAt(0) == "Ι") {
                    workingString = "Ἴ";
                }
                if (workingString.charAt(0) == "ο") {
                    workingString = "ὄ";
                }
                if (workingString.charAt(0) == "Ο") {
                    workingString = "Ὄ";
                }
                if (workingString.charAt(0) == "υ") {
                    workingString = "ὔ";
                }
            }
        } else {
